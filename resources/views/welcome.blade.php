<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8" />
    <meta http-equiv="X-UA-Compatible" content="IE=edge" />
    <meta name="viewport" content="width=device-width, initial-scale=1.0" />
    <title>Crypto Currency</title>
    <link rel="stylesheet" href="../assets/css/index.css" />
  </head>
  <body>
    <header>
      <nav>
        <img src="../assets/img/logo.png" />
        <ul>
          <li><a href="">Home</a></li>
          <li><a href="#services">Services</a></li>
          <li><a href="#token">Token</a></li>
          <li><a href="#convertor">Convertor</a></li>
          <li><a href="#roadmap">Road map</a></li>
          <li><a href="#news">News</a></li>
        </ul>
        @guest
          @if (Route::has('login'))
            <li class="nav-item">
              <a class="nav-link" href="{{ route('login') }}"><button>{{ __('Login') }}</button></a>
            </li>
          @endif
        @endguest
      </nav>
      <section>
        <div class="content">
          <h1>
            Buy and Sell <br />
            Digital Currency <br />Here.
          </h1>
          <p>
            Cryotis it is Cryptocurrency ICO. Do not miss it! 
            <br />
             Buy and sell cryptocurrencies with 20+ fiat currencies using bank transfers or your credit/debit card.
          </p>
          <a href="signup.php"><button>Sign Up</button></a>
        </div>
        <div class="img_data">
          <img src="../assets/img/Crypto Earth 1.png" alt="" />
        </div>
      </section>
    </header>

    <aside id='token'>
      <h1>What Is ICO Cryotis?</h1>
      <p>
         At ICO Cryotis, we believe that everyone should have  the freedom to earn, hold, spend, <br>share and give their money - no matter who you are or where you come from.
      </p>
      <img src="../assets/img/grow line.png" />
      <div class="time">
      </div>

      <button>BUY TOKEN NOW</button>
    </aside>

    <div class="app">
      <div class="app_img_bx">
        <img src="../assets/img/Crypto App.png" alt="" />
      </div>
      <div class="app_contect_bx">
        <h1>Awesome App Which Works For Your Business</h1>
        <p>
        Trade on the go. Anywhere, anytime.
        <br/>Download the ICO Cryotis
        </p>
        <img src="../assets/img/allstore.png" alt="" style="width:300px;height:100px"/>
      </div>
    </div>

    <div class="converter" id="convertor">
      <img src="../assets/img/crypto bg.jpg" class="pic" />
      <h1 class="crypto-con">Crypto Convertor</h1>

      <div class="cards">
        <div class="card">
          <div class="select">
            <h4 id="coin_first">B</h4>
            <div class="select1">
              <h6>GIVE</h6>
              <select id="coin" value="bitcoin">
                <option value="Bitcoin">Bitcoin</option>
                <option value="Ethereum">Ethereum</option>
                <option value="Tether">Tether</option>
                <option value="BNB">BNB</option>
              </select>
            </div>
          </div>
          <div class="enter_bitcoin">
            <input type="text" value="0.1" id="crypto" />
            <h6 id="btc">BTC</h6>
          </div>

          <p>Rate expressed in Digital Currency</p>
        </div>
        <div class="card2">
          <svg
            xmlns="http://www.w3.org/2000/svg"
            width="16"
            height="16"
            fill="currentColor"
            class="bi bi-arrow-left-right"
            viewBox="0 0 16 16">
            <path
              fill-rule="evenodd"
              d="M1 11.5a.5.5 0 0 0 .5.5h11.793l-3.147 3.146a.5.5 0 0 0 .708.708l4-4a.5.5 0 0 0 0-.708l-4-4a.5.5 0 0 0-.708.708L13.293 11H1.5a.5.5 0 0 0-.5.5zm14-7a.5.5 0 0 1-.5.5H2.707l3.147 3.146a.5.5 0 1 1-.708.708l-4-4a.5.5 0 0 1 0-.708l4-4a.5.5 0 1 1 .708.708L2.707 4H14.5a.5.5 0 0 1 .5.5z" />
          </svg>
        </div>

        <div class="card">
          <div class="select">
            <h4 id="gov_coins">$</h4>
            <div class="select1">
              <h6>GET</h6>
              <select id="coins" value="bitcoin">
                <option value="Dollar">Dollar</option>
                <option value="Rupee">Rupee</option>
                <option value="Paund">Paund</option>
                <option value="Riyal">Riyal</option>
              </select>
            </div>
          </div>
          <div class="enter_bitcoin">
            <input type="text" value="1997.45" id="gov" />
            <h6 id="usd">USD</h6>
          </div>

          <p>Rate expressed in Government Currency</p>
        </div>
      </div>
    </div>

    <div class="rocket" id="services">
      <img src="./img/rocket_bg.png" class="pic" />
      <div class="rocket_content">
        <h1>Cryotis Perfect Services That Works For You!</h1>
        <p>
          Cryotis Perfect Services That Works For You!
        </p>
        <ul>
          <li>Mobile App:</li>
          <li>Exchange Services:</li>
          <li>Insurance Protection</li>
          <li>Credit Card User:</li>
          <li>Secure Storage:</li>
          <li>Multycurrancy Wallet</li>
        </ul>

        <button>DISCOVER NOW</button>
      </div>
      <div class="rocket_img">
        <img src="../assets/img/Crypto Rocket.png" alt="" />
      </div>
    </div>

    <div class="road_map" id="roadmap">
      <img src="../assets/img/rocket_bg.png" class="pic1" />
      <h1>Crypto Road Map</h1>
      <p>
        Cryotis Road Map!
      </p>
      <div class="map">
        <div class="card">
          <div class="title">
            <h6>25 APR 2022</h6>
            <h4>Stats Center Beta</h4>
          </div>
          <h5></h5>
          <h2></h2>
        </div>
        <div class="card">
          <div class="title">
            <h6>25 APR 2022</h6>
            <h4>Stats Center Beta</h4>
          </div>
          <h5></h5>
          <h2></h2>
        </div>
        <div class="card">
          <div class="title">
            <h6>25 APR 2022</h6>
            <h4>Stats Center Beta</h4>
          </div>
          <h5></h5>
          <h2></h2>
        </div>
        <div class="card">
          <div class="title">
            <h6>25 APR 2022</h6>
            <h4>Stats Center Beta</h4>
          </div>
          <h5></h5>
          <h2></h2>
        </div>
        <div class="card">
          <div class="title">
            <h6>25 APR 2022</h6>
            <h4>Stats Center Beta</h4>
          </div>
          <h5></h5>
          <h2></h2>
        </div>
        <div class="card">
          <div class="title">
            <h6>25 APR 2022</h6>
            <h4>Stats Center Beta</h4>
          </div>
          <h5></h5>
          <h2></h2>
        </div>
      </div>
    </div>

    <div class="man">
      <img src="../assets/img/Man_bg.png" class="pic3" />
      <div class="man_img">
        <img src="../assets/img/Crypto Man.png" alt="" />
      </div>
      <div class="man_content">
        <h1>Review From Our Clients About Cryotis ICO</h1>
        <p>
           Cryotis ICO helped me so much understanding the world of Crypto Currencye's. <br>
           So for me is a 10/10 website it helped me learn so much plus earn so much money!
        </p>
        <div class="client_cole">
          <div class="cont">
            <div class="star">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
                fill="currentColor"
                class="bi bi-star-fill"
                viewBox="0 0 16 16">
                <path
                  d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z" />
              </svg>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
                fill="currentColor"
                class="bi bi-star-fill"
                viewBox="0 0 16 16">
                <path
                  d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z" />
              </svg>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
                fill="currentColor"
                class="bi bi-star-fill"
                viewBox="0 0 16 16">
                <path
                  d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z" />
              </svg>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
                fill="currentColor"
                class="bi bi-star-fill"
                viewBox="0 0 16 16">
                <path
                  d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z" />
              </svg>
              <svg
                xmlns="http://www.w3.org/2000/svg"
                width="16"
                height="16"
                fill="currentColor"
                class="bi bi-star-fill"
                viewBox="0 0 16 16">
                <path
                  d="M3.612 15.443c-.386.198-.824-.149-.746-.592l.83-4.73L.173 6.765c-.329-.314-.158-.888.283-.95l4.898-.696L7.538.792c.197-.39.73-.39.927 0l2.184 4.327 4.898.696c.441.062.612.636.282.95l-3.522 3.356.83 4.73c.078.443-.36.79-.746.592L8 13.187l-4.389 2.256z" />
              </svg>
            </div>
            <h4>Alina Delz</h4>
            <p>TRADER</p>
          </div>
          <div class="img">
            <img src="../assets/img/user.jpg" />
          </div>
        </div>
      </div>
    </div>


    <div class="news" id="news">
        <h1>Crypto Latest News</h1>
        <div class="cards">
            <div class="card">
                <h6>BITCOIN</h6>
                <h2>Crypto Really <br> Wants a Bitcoin ETF</h2>
                <p>Bitcoin is a newcomer to the world of exchange-traded funds (ETFs). Bitcoin ETFs allow investors to get exposure to the enticing potential of BTC without having to safely store it</p>
                <h5>AUG, 23 2022</h5>
            </div>
            <div class="card">
                <h6>CRYPTO</h6>
                <h2>Crypto Stock Exchange <br> Operator Pleads</h2>
                <p>The man behind two now-defunct cryptocurrency investment sites has pled guilty to securities fraud and obstruction of justice charges. </p>
                <h5>APR, 03 2022</h5>
            </div>
            <div class="card">
                <h6>BLOCKCHAIN</h6>
                <h2>Don't Let the Crypto <br> Circus Fool You</h2>
                <p>Blockchain.com is a cryptocurrency financial services company. The company began as the first Bitcoin blockchain explorer in 2011 and later created a cryptocurrency wallet</p>
                <h5>JUL, 15 2022</h5>
            </div>
        </div>
        <div class="btn">
            <button>READ MORE</button>
        </div>
       
    </div>

    <div class="footer">
      <img src="../assets/img/logo.png" />

      <div>
        <h1 class="h1">© Vesa Susuri 2022 / Cryotis ICO</h1>
      </div>
    </div>
    <script src="../assets/js/app.js"></script>
  </body>
</html>
